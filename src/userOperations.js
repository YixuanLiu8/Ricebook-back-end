...
const postLogin = async (req, res) =>{
    ...
    if (hash === user.hash) {
        ...
        res.cookie(cookieKey, sid, { maxAge: expireTime * 1000, httpOnly: false,sameSite: 'none', secure: true });
...
const putLogout = (req,res) => {
    ...
    redis.del(sid);
    res.cookie(cookieKey, sid, { maxAge: 5 * 1000, httpOnly: false,sameSite: 'none', secure: true });
...
const putPassword= async (req,res)=>{
    ...
    let salt = req.user + new Date().getTime();
    let hash = md5(salt + password);
    ...
    return res.json({result: 'success', username: req.user});
}
module.exports = (app) => {
